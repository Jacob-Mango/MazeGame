...
module.exports = function(workerToSceneMessageHandler, Ammo) {
	var //transferableMessage = self.webkitPostMessage || self.postMessage,
		transferableMessage = function(x) {
			workerToSceneMessageHandler({ data: x });
		},
		// enum
		MESSAGE_TYPES = {
			WORLDREPORT: 0,
	...
	createShape = function(description) {
		...
		switch (description.type) {
			...
			case "heightfield":
				...
				shape = new Ammo.btHeightfieldTerrainShape(description.xpts, description.ypts, ptr, 1, -description.absMaxHeight, description.absMaxHeight, 2, 0, false);
	...
	public_functions.init = function(params) {
		...
		world = new Ammo.btCollisionWorld(dispatcher, broadphase, collisionConfiguration);
	...
	public_functions.addObject = function(description) {
		...
		colObject = new Ammo.btCollisionObject();

		colObject.setCollisionShape(shape);

		colObject.setWorldTransform(_transform);
		colObject.setUserPointer(description.id);

		world.addCollisionObject(colObject);

		colObject.id = description.id;
		_objects[colObject.id] = colObject;

		var ptr = colObject.a != undefined ? colObject.a : colObject.ptr;
		_objects_ammo[ptr] = colObject.id;
		_num_objects++;

		transferableMessage({ cmd: "objectReady", params: colObject.id });
	};
  ...
  */
	...
	public_functions.removeObject = function(details) {
		world.removeCollisionObject(_objects[details.id]);
	...
	public_functions.updateTransform = function(details) {
		...
		_transform = _object.getWorldTransform();
		...
		switch (details.type) {
			...
			case "dof":
				...
				if (details.objectb) {
					...
					constraint = new Ammo.btGeneric6DofConstraint(_objects[details.objecta], _objects[details.objectb], transforma, transformb);
				} else {
					constraint = new Ammo.btGeneric6DofConstraint(_objects[details.objecta], transforma);
				}
				Ammo.destroy(transforma);
				if (transformb != undefined) {
					Ammo.destroy(transformb);
	...
	public_functions.simulate = function simulate(params) {
		if (world) {
			...
			for (var i = 0; i < numManifolds; i++) {
				...
				var obA = _objects[_objects_ammo[contactManifold.getBody0()]];
				var obB = _objects[_objects_ammo[contactManifold.getBody1()]];
	...
	reportWorld = function() {
		...
		for (index in _objects) {
			if (_objects.hasOwnProperty(index)) {
				...
				_vector = new Ammo.btVector3(0, 0, 0);
