	...
	Eventable.prototype.dispatchEvent = function(event_name) {
		...
		if (this._eventListeners.hasOwnProperty(event_name)) {
			for (i = 0; i < this._eventListeners[event_name].length; i++) {
				return this._eventListeners[event_name][i].apply(this, parameters);
			}
		}
		return false;
	};
...
*/
	...
	Physijs.Scene = function(params) {
				...
				if (data.cmd) {
					...
					switch (data.cmd) {
						...
						case "vehicle":
							...
							break;
						case "collision":
							var obA = self._objects[data.obA];
							var obB = self._objects[data.obB];
		...
		this._worker = require("./physijs_worker")(workerToSceneMessageHandler, Ammo);
		...
		for (i = 0; i < num_objects; i++) {
			...
			if (object.__dirtyRotation === false) {
				if (object.useQuaternion) {
					...
					object.rotation = getEulerXYZFromQuaternion(data[offset + 4], data[offset + 5], data[offset + 6], data[offset + 7]);
				}
			}
	...
	Physijs.Scene.prototype._updateVehicles = function(data) {
		...
		for (i = 0; i < (data.length - 1) / VEHICLEREPORT_ITEMSIZE; i++) {
			...
			if (wheel.useQuaternion) {
				...
				wheel.rotation = getEulerXYZFromQuaternion(data[offset + 5], data[offset + 6], data[offset + 7], data[offset + 8]);
			}
		}
	...
	Physijs.Scene.prototype.addConstraint = function(constraint, show_marker) {
		...
		if (show_marker) {
			...
			switch (constraint.type) {
				...
				case "slider":
					marker = new THREE.Mesh(new THREE.CubeGeometry(10, 1, 1), new THREE.MeshNormalMaterial());
					marker.position.copy(constraint.positiona);
